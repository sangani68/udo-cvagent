...
export async function textToCvJson(text: string, opts?: LlmOpts): Promise<LlmCv> {
  ...
  const apiKey = (opts?.apiKey ?? process.env.AZURE_OPENAI_API_KEY ?? process.env.AZURE_OPENAI_KEY ?? "").trim();
  const deployment = (opts?.deployment ?? process.env.AZURE_OPENAI_CHAT_DEPLOYMENT ?? process.env.AZURE_OPENAI_DEPLOYMENT ?? "").trim();
  const apiVersion = (opts?.apiVersion ?? process.env.AZURE_OPENAI_CHAT_API_VERSION ?? "2024-10-01-preview").trim();
  ...
  if (!res.ok) {
    ...
    throw new Error(`Azure OpenAI ${res.status} for "${deployment}" @ "${endpoint}" v=${apiVersion}. ${msg}`);
  }
  ...
  const fix = (arr: any[], f: (x: any) => any) => Array.isArray(arr) ? arr.map(f).filter(Boolean) : [];
  ...
  return {
    candidate: {
      ...
      skills: fix(c?.skills, (s) => (typeof s === "string" ? s.trim() : String(s?.name || "").trim())),
...
function clamp(n: any) { const x = Number(n); return Number.isFinite(x) ? Math.max(0, Math.min(1, x)) : 0.6; }
function estimateConfidence(raw: any) {
  const c = raw?.candidate ?? {}; let s = 0, t = 0;
  const add = (ok: any, w = 1) => { t += w; if (ok) s += w; };
  add(!!c?.name, 2); add((c?.experience?.length ?? 0) > 0, 2); add((c?.skills?.length ?? 0) > 2, 1);
  add(!!c?.contacts?.email, 1); add((c?.education?.length ?? 0) > 0, 1);
  return t ? s / t : 0.5;
}
function inferGaps(raw: any) {
  const c = raw?.candidate ?? {}; const g: string[] = [];
  ...
  else c.experience.forEach((e: any, i: number) => {
