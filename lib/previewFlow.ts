...
export function safeFileBase(s: string) {
  return (s || "candidate").replace(/\s+/g, "_").replace(/[^A-Za-z0-9._-]/g, "_").slice(0, 80);
}
...
export async function translateCv(cv: any, to: "en" | "fr" | "de" | "nl"): Promise<CvData> {
...
export function buildHtmlSnapshot(data: CvData, _template: TemplateId) {
  ...
  if (metaBits.length) lines.push(`<p><small>${metaBits.join(" • ")}</small></p>`);
  ...
  if (Array.isArray(c.experiences) && c.experiences.length) {
    ...
    for (const e of c.experiences) {
      ...
      lines.push(`<h3>${head}${dates ? ` <small>(${dates})</small>` : ""}</h3>`);
  ...
  if (Array.isArray(c.education) && c.education.length) {
    ...
    for (const ed of c.education) {
      const head = [ed?.title, ed?.org].filter(Boolean).map(esc).join(" — ");
      const dates = [ed?.start, ed?.end].filter(Boolean).map(esc).join(" – ");
      lines.push(`<h3>${head}${dates ? ` <small>(${dates})</small>` : ""}</h3>`);
      if (ed?.details) lines.push(`<p>${esc(ed.details)}</p>`);
    }
  }
  ...
  if (Array.isArray(c.certifications) && c.certifications.length) {
    ...
    for (const cert of c.certifications) {
      const t = [cert?.name, cert?.date].filter(Boolean).map(esc).join(" — ");
      lines.push(`<li>${t}</li>`);
    }
    lines.push(`</ul>`);
  }
  ...
  if (Array.isArray(c.languages) && c.languages.length) {
    ...
    lines.push(
      `<p>${c.languages
        .map((l) => [l?.name, l?.level].filter(Boolean).map(esc).join(" — "))
        .join(", ")}</p>`
    );
  }
