...
export async function POST() {
  try {
    ...
    await put(`/indexes/${blobIndex}`, {
      name: blobIndex,
      fields: [
        { name: "id", type: "Edm.String", key: true, filterable: true, sortable: true }, // <-- SAFE KEY
    ...
    // Recreate indexer — map blob path → id using base64Encode
      ...
      outputFieldMappings: [
        {
          ...
          targetFieldName: "id",
          mappingFunction: { name: "base64Encode" }, // <-- KEY FIX
        },
      ],
    });
    ...
    return NextResponse.json({ ok: true, note: "Blob index reset (safe key) + indexer kicked", ran });
  } catch (e: any) {
