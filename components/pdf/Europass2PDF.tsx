...
export default function Europass2PDF({ data }: { data: CvData }) {
  ...
  const showCertIssuer = certifications.some((cert: any) => !!s(cert?.issuer || cert?.org || cert?.company).trim());
  const showCertStart = certifications.some((cert: any) => !!s(cert?.start).trim());
  const showCertEnd = certifications.some((cert: any) => !!s(cert?.end || cert?.validUntil).trim());
  const certNameWidth =
